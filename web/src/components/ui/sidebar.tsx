...
export function Sidebar({ className, ...props }: SidebarProps) {
  ...
  return (
    <aside
      ...
      className={cn(
        " top-0 px-9  left-0 flex h-[calc(100vh-4rem)] flex-col bg-white dark:bg-stone-950 border-r border-border transition-all duration-300 z-30",
        isMobile ? (open ? "translate-x-0" : "-translate-x-full") : "",
        className
      )}
      style={{
        width: isMobile ? (open ? mobileWidth : "0") : open ? width : "320px",
      }}
      {...props}
    />
  );
...
export function SidebarHeader({ className, ...props }: SidebarHeaderProps) {
  return (
    <header
      className={cn(" flex w-full h-[100px] items-center  ", className)}
      {...props}
    />
  );
}
...
export function SidebarMenu({ className, ...props }: SidebarMenuProps) {
  return <ul className={cn("space-y-2", className)} role="menu" {...props} />;
}
...
export function SidebarMenuItem({ className, ...props }: SidebarMenuItemProps) {
  return (
    <li className={cn("list-none", className)} role="menuitem" {...props} />
  );
}
  ...
  "flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition-all duration-300 hover:bg-primary/5 hover:text-primary group",
  {
    variants: {
      variant: {
        ...
        link: "bg-transparent text-primary underline-offset-4 hover:bg-transparent hover:underline data-[active]:bg-transparent data-[active]:text-primary",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, asChild = false, active, type, ...props }, ref) => {
