...
export function GenerateCourseCard({ className }: GenerateCourseCardProps) {
  return (
    <Link href="/dashboard/create/wizard" className="block">
      <Card
        className={cn(
          "h-20 transition-all duration-200 shadow-sm hover:shadow-lg border border-border/60 dark:border-border/40 bg-card/60 backdrop-blur-sm hover:border-accent/30 rounded-2xl cursor-pointer group",
          className
        )}
      >
