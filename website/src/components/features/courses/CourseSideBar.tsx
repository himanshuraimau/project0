...
import { useCourseProgress } from "@/lib/contexts/course-progress-context";
...
function ChapterItem({
  ...
  const { updateChapterProgress, refreshProgress, chapterProgress } =
    useCourseProgress();
    ...
    <SidebarMenuItem>
      ...
      >
        ...
        >
          <span
            className={cn(
              "w-5 h-5 rounded-sm flex items-center justify-center shrink-0 mr-3 text-xs font-semibold",
              isCompleted
                ...
                {!isCollapsed && (
                  <div>
                    <SidebarGroupLabel className="px-3 py-2 mb-2 rounded-md border border-gray-200/50 dark:border-gray-700/50 flex items-center gap-2 overflow-hidden">
                      <div className="w-6 h-6 rounded-md bg-primary text-white flex items-center justify-center text-xs font-bold shrink-0">
                        {unitIndex + 1}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-stone-200 dark:bg-stone-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${unitProgressData.progressPercentage}%`,
                            }}
                          />
                        </div>
                        <span className="text-[10px] text-stone-500 dark:text-stone-400 font-medium">
                          {unitProgressData.completedChapters}/
                          {unitProgressData.totalChapters}
                        </span>
                      </div>
                    </div>
                  </div>
